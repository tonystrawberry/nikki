...
The architecture was new to me, and I came away a fan. Three layers: a frontend talking to a **BFF** over GraphQL, and the BFF talking to a **Ruby on Rails** backend over gRPC. The whole thing was **schema-first** — we defined the contract in proto files, and the code on both the BFF and the backend side was generated from that schema. It meant the layers couldn't quietly drift apart: change the schema, regenerate, and the compiler tells you exactly what you broke. I'd never built that way before, and it worked really well.

A feature I was especially proud to build was a reservation system. "Book a time slot" sounds simple; it isn't. It integrated with Google Calendar to check availability and actually reserve the slot, and it sat on top of a tangle of application-side rules — roles, smart assignment of attendees, the set of possible times — all of which had to agree before a reservation could land. Getting those constraints to line up cleanly was the kind of problem I like: small on the surface, deep once you're in it.

Another feature I was glad to build was the analytics — aggregating a lot of data to surface daily, weekly, and three-month views. I reached for **Google BigQuery**, an OLAP database perfectly suited to that kind of work. My time at Overflow paid off here: I'd designed and built something very similar before, so I knew the shape of the problem going in. An aggregation job ran once a day in the OLAP database, and its results were loaded back into the OLTP store — keeping read requests for the aggregated data nice and quick. It worked without a hitch.
...
- **Worked in a schema-first, three-layer architecture** — a GraphQL BFF between the frontend and a Ruby on Rails gRPC backend, with code generated on both the BFF and backend sides from shared proto schemas — a way of building I hadn't used before and came to really like.
- **Built a reservation system integrated with Google Calendar** (availability checks and slot booking) on top of complex application-side rules — roles, smart attendee assignment, and constrained sets of possible times that all had to agree before a reservation could land.
- **Built an analytics feature on Google BigQuery** (OLAP) that aggregated large volumes of data into daily, weekly, and three-month views — a daily aggregation job in the OLAP database loaded results back into the OLTP store to keep reads fast, drawing on similar work I'd done at Overflow.
